...
class EmbeddingStore:
    ...
    Stores records in memory without requiring an external database.
    ...
    def _make_record(self, doc: Document) -> dict[str, Any]:
        ...
        if not metadata.get("doc_id"):
            parent, marker, index = doc.id.rpartition("#")
            metadata["doc_id"] = parent if parent and marker and index.isdigit() else doc.id
        return {
            "id": doc.id,
            "content": doc.content,
            "metadata": metadata,
    ...
    def add_documents(self, docs: list[Document]) -> None:
        ...
        One Document becomes one record; chunking is the caller's responsibility.
        """
